...
double shitomasi_min, shitomasi_max;
int sm_qualityLevel = 30;
int qualityLevel = 30;
int max_count = 100;
void CustomHarris_Demo(int, void*);
void Costum_tomasi(int, void*);
Mat harris_dst;
Mat src, gray_src;
Mat harrisRspimg;
Mat shiIomasiRsp;
...
int main() {
	...
	Mat sgiTomasi = Mat::zeros(src.size(), CV_32FC1);
	cornerMinEigenVal(gray_src, sgiTomasi, blockSize, ksize, 4);
	minMaxLoc(sgiTomasi, &shitomasi_min, &shitomasi_max, 0, 0, Mat());
	namedWindow("output1", CV_WINDOW_AUTOSIZE);
	createTrackbar("demo1", "output1", &sm_qualityLevel, max_count, Costum_tomasi);
	Costum_tomasi(0, 0);
...
void Costum_tomasi(int, void*) {
	...
	Mat resultImg = src.clone();
	float t = shitomasi_min + (((double)sm_qualityLevel) / max_count)*(shitomasi_max - shitomasi_min);
	for (int row = 0; row < src.rows; row++) {
		for (int col = 0; col < src.cols; col++) {
			float v = shiIomasiRsp.at<float>(row, col);
			if (v > t) {
				circle(resultImg, Point(col, row), 2, Scalar(0, 0, 255), 2, 8, 0);
			}
		}
